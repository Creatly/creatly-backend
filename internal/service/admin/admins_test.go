...
import (
	...
	"github.com/zhashkevych/creatly-backend/internal/service/admin"
...
func mockAdminService(t *testing.T) (*admin.AdminsService, *mock_repository.MockAdmins, *mock_repository.MockSchools) {
	t.Helper()
...
func TestNewAdminsService_SignInErr(t *testing.T) {
	...
	res, err := adminService.SignIn(ctx, admin.SchoolSignInInput{})

	require.True(t, errors.Is(err, errInternalServErr))
	require.Equal(t, admin.Tokens{}, res)
}
...
func TestNewAdminsService_SignIn(t *testing.T) {
	...
	require.IsType(t, admin.Tokens{}, res)
}
